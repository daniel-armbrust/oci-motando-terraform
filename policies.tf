...
module "policies_motando" {
    ...
    statements = [
       ...
       "Allow dynamic-group ${module.dyngrp_motando.name} to manage objects in compartment CSM-BR:daniel.armbrust:${module.cmp_motando.name} where any {target.bucket.name='${module.gru_bucket-anuncio_prd.name}', target.bucket.name='${module.gru_bucket-anuncio-tmp_prd.name}, target.bucket.name='${module.gru_bucket-usuario_prd.name}'}",
